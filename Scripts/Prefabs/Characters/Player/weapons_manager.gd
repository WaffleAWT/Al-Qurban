...
#Data
enum WEAPON_SLOTS {SHOTGUN, RIFLE, BAZOOKA}
var slots_unlocked : Dictionary = {
	WEAPON_SLOTS.SHOTGUN : true,
	WEAPON_SLOTS.RIFLE : true,
	WEAPON_SLOTS.BAZOOKA : true
}
...
func init(_fire_point : Node3D, _bodies_to_exclude : Array) -> void:
	...
	weapons[WEAPON_SLOTS.RIFLE].fired.connect(alert_nearby_enemies)
	weapons[WEAPON_SLOTS.BAZOOKA].fired.connect(alert_nearby_enemies)
