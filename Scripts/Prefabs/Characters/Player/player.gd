...
func die():
	...
	add_child(splatter)
	splatter.global_position = $Marker3D.global_position

func eating_sound() -> void:
	if not off_hand.current_item == off_hand.ITEM.EMPTY:
		AudioManager.play_sound(AudioManager.eating)
...
func play_shotgun() -> void:
	AudioManager.play_sound(AudioManager.shotgun)

func play_rifle() -> void:
	AudioManager.play_sound(AudioManager.rifle)

func play_bazooka() -> void:
	AudioManager.play_sound(AudioManager.bazooka)
